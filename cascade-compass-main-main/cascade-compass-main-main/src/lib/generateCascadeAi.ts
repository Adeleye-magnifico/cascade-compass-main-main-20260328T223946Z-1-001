...
async function callGeminiCascade(systemPrompt: string, userPrompt: string): Promise<string> {
  ...
  if (result.error?.message) {
    console.error("Gemini API error body:", result.error);
    throw new Error("AI generation failed. Please try again.");
  }

  const candidate = result.candidates?.[0];
  if (!candidate) {
    throw new Error("AI returned no output (request may have been blocked). Please adjust inputs and try again.");
  }
  if (candidate.finishReason === "MAX_TOKENS") {
    throw new Error("AI output was truncated. Try a smaller form or set GEMINI_CASCADE_MODEL to a model with a higher output limit.");
  }
...
export async function runCascadeAi(formData: JobAnalysisInput): Promise<CascadeOutput> {
  ...
  const content = await callGeminiCascade(SYSTEM_PROMPT, userPrompt);
...
/** @deprecated Use `runCascadeAi`. Kept so existing imports keep working. */
export const runLovableCascade = runCascadeAi;
